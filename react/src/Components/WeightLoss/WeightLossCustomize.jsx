    ...
    return (
        <div className="container mx-auto grid grid-cols-2 gap-10">
            <div className=" p-10 rounded-lg">
                    ...
                    <div className="flex gap-3 mt-4">

                        {
                            durationPlan.map((item, idx) => <>
                                <button onClick={() => selectDuration(item)} key={idx} className={`border  py-2 px-3 rounded-md text-md ${duration === item ? ' border-primary bg-primary text-white' : 'border-secondary bg-white text-black'}`}> {item}</button>
                            </>)
                        }
                    </div>
                </div>
                    ...
                    <div className="flex gap-3 mt-4">

                        {
                            mealsTime.map((item, idx) => <>
                                <button onClick={() => selectMealTime(item)} key={idx} className={`border  py-2 px-4 rounded-md text-md ${mealTime === item ? ' border-primary bg-primary text-white' : 'border-secondary bg-white text-black'}`}> {item}</button>
                            </>)
                        }
                    </div>
                </div>
                ...
                <div className="my-10">
                    <button onClick={handleNext} className="btn  py-2 px-6 rounded-md  bg-white text-primary border-2 border-primary hover:bg-gray-600 hover:border-gray-600 hover:text-white">Next</button>
                </div>
